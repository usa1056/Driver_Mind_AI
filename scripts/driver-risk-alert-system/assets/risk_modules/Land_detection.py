...
def main(video_path):
    ...
    while cap.isOpened():
        ...
        if not ret:
            break
        frame_with_colors, _, _ = process_frame(frame)
        cv2.imshow("Lane with Multi-color Zones", frame_with_colors)
...
def get_lane_roi_dynamic(left_line, right_line, frame_shape):
    """
    根據左右車道線的間距動態計算紅橙綠區塊的多邊形座標
    """
    if left_line is None or right_line is None:
        return {}
    ...
    # 根據寬度調整 ROI 深度（可依場景微調）
    scale = min(max(lane_width / 400, 0.5), 0.6)  # 正常寬道約 400px，窄巷會變小
...
def process_frame(frame):
    ...
    回傳 ROI 區域
    """
    global left_line_history, right_line_history # 加入才可以 append()
    ...
    roi_dict = get_lane_roi_dynamic(left_line, right_line, frame.shape)
    scene_valid = is_valid_lane_scene(left_line, right_line, frame.shape)
    return frame_with_colors, roi_dict, scene_valid
